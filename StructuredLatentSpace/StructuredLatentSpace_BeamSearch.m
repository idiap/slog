...
function [ closest ] = StructuredLatentSpace_BeamSearch( xi, A , X,k, p, B )
...
for kk=1:k,
    ...
    for b=1:it,
       ...
       for ii=1:length(zeroElements),
           ...
           [f ]=NoneLinearLatentSpaceEval( thisRound(index,:), XA, xiA, p);
...
maxgain=0;
for bb=1:B,
    val = NoneLinearLatentSpaceEval( w(bb,:),XA, xiA, p);
    if val > maxval,
        maxval = val;
        maxw = w(bb,:);
        maxgain = gain(bb,:);
...
function [f] = NoneLinearLatentSpaceEval(w , XA, xiA, p)
    
    f =    sum(sum(repmat(w',1 , size(XA,2)).*XA .* repmat(xiA, size(XA,1),1)).^p);
