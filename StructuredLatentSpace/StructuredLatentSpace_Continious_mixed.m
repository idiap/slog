...
function [ closest ] = StructuredLatentSpace_Continious_mixed( xi, A , X, lambda, p, B )
...
for b=1:B,
    ...
    w0 = rand(1, size(X,1));
    [f df] = NoneLinearLatentSpaceMixed(w0, XA, xiA, p , lambda);
    [w fval] = fmincon(@(w)NoneLinearLatentSpaceMixed(w, XA, xiA, p , lambda),w0,[],[],[],[],zeros(size(w0)),ones(size(w0)),[],options);
...
function [f df] = NoneLinearLatentSpaceMixed(w, XA, xiA, p ,q, lambda, a)

f1 = sum(nthroot(sum(repmat(w',1 , size(XA,2)).*XA .* repmat(xiA, size(XA,1),1)-a).^q, p));
f2 = norm(lambda .*w, 1);


f = -f1+  f2;

df1 = (q/p)*(nthroot((sum(repmat(w',1 , size(XA,2)).*XA .* repmat(xiA, size(XA,1),1) - a).^(q-p)), p));
